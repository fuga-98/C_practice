...
//     HH、MM、SSはそれぞれ時間、分、秒を2桁で表した文字列
    string to_str(){
        string h_s, m_s, s_s;
        if (hour >= 10){h_s = to_string(hour);} else {h_s ="0" + to_string(hour);}
        if (minute >= 10){m_s = to_string(minute);} else {m_s ="0" + to_string(minute);}
        if (second >= 10){s_s = to_string(second);} else {s_s ="0" + to_string(second);}
        return h_s + ":" + m_s +":"+ s_s;
    }
...
//     diff_second の値は -86400 ~ 86400 の範囲を取とりうる
    void shift(int diff_second){
        ...
        int changed_second = now_second + diff_second;
        if (changed_second < 0){
            changed_second +=86400;
        }
        if (changed_second >= 86400){
            changed_second -= 86400;
        }
        int h = changed_second / 3600;
        int m = (changed_second % 3600) / 60;
        int s = changed_second % 60;
        set(h, m, s);
    }
};
